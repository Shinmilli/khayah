...
function fallbackLatest(): YoutubeLatest {
  return {
    ...
    publishedAt:
      process.env.YOUTUBE_FALLBACK_PUBLISHED_AT?.trim() || DEFAULT_FALLBACK_PUBLISHED_AT,
    channelUrl: channelUrl(),
  }
}

function rssUrls(channelId: string): string[] {
...
async function fetchRssForChannel(channelId: string): Promise<string> {
  let lastError: unknown

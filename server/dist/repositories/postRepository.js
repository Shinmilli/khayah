...
const mockPosts = [
    ...
    /* 워드프레스 공지 목록 이관용 샘플 (MOCK_DATA / DB 없을 때) */
    {
        ...
        postExcerpt: '',
        postContent: '<p>경기청년 기후특사단 캄보디아팀 조기귀국과 관련한 안내입니다. 자세한 내용은 추후 공지에 따라 진행됩니다.</p>',
        postParent: 0,
        menuOrder: 0,
    ...
    },
    {
        ...
        postExcerpt: '',
        postContent: '<p>2025년 경기청년 기후특사단 최종합격자 발표 및 사전 OT 일정을 안내합니다.</p>',
        postParent: 0,
        menuOrder: 0,
...
exports.postRepository = {
    ...
    async findPublishedPages(options) {
        ...
        if (!prisma_1.prisma) {
            const publishedPages = mockPosts
                ...
                .sort((a, b) => {
                if (a.postParent !== b.postParent)
                    return a.postParent - b.postParent;
    ...
    async findPostBySlug(slug) {
        ...
        if (!prisma_1.prisma) {
            return (mockPosts.find((p) => p.postStatus === 'publish' && p.postType === 'post' && p.postName === decoded) ?? null);
        }
    ...
    async findPageBySlug(slug) {
        const decoded = decodeURIComponent(slug);
        if (!prisma_1.prisma) {
            const page = mockPosts.find((p) => p.postStatus === 'publish' && p.postType === 'page' && p.postName === decoded);
            if (!page)
                return null;
            return {
                id: page.id,
        ...
        const page = await prisma_1.prisma.post.findFirst({
            where: {
                postStatus: 'publish',
                postType: 'page',
                postName: decoded,

...
exports.looksLikeImageBuffer = looksLikeImageBuffer;
exports.looksLikePdfBuffer = looksLikePdfBuffer;
/** Detect obvious image payloads so they are not stored under documents/ as raw. */
function looksLikeImageBuffer(buf, mimeType, originalName) {
    const mime = (mimeType ?? '').toLowerCase();
    if (mime.startsWith('image/'))
        return true;
    const ext = (originalName ?? '').toLowerCase();
    if (/\.(jpe?g|png|gif|webp|bmp|heic|heif|tiff?)$/i.test(ext))
        return true;
    if (buf.length >= 3) {
        // JPEG
...
//# sourceMappingURL=uploadSniff.js.map
...
if( ! function_exists( 'mfn_breadcrumbs' ) )
{
	function mfn_breadcrumbs( $class = false ){
		...
		// Blog | Category --------------------------------
			...
			if( $cat && $cat->parent ){
				$breadcrumbs[] = get_category_parents( $cat->parent, false, $separator );
				//echo $breadcrumbs[1];
			}
		...
		// Single -----------------------------------------
			...
			if( get_post_type() != 'post' ){
				...
				if( $slug['slug'] == mfn_opts_get( 'portfolio-slug', 'portfolio-item' ) && $portfolio_page_id ){
					$breadcrumbs[] = 'eee';/*'<a href="' . get_page_link( $portfolio_page_id ) . '">'. get_the_title( $portfolio_page_id ) .'</a>';	*/
				}
		...
		// Page with parent -------------------------------
			...
			while( $parent_id ) {
				$page = get_page( $parent_id );
				$parents[] = get_the_title( $page->ID );/*'<a href="' . get_permalink( $page->ID ) . '">' . get_the_title( $page->ID ) . '</a>';*/
				$parent_id  = $page->post_parent;
			}
			$parents = array_reverse( $parents );
			$breadcrumbs = array_merge_recursive($breadcrumbs, $parents);
